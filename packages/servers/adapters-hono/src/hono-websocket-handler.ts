import {  FlowServer, UploadServer, type UploadEvent,  } from "@uploadista/core";
...
const extractWebSocketRequest = (
  c: Context,
  baseUrl: string,
): WebSocketConnectionRequest | { type: "invalid-path"; expectedPrefix: string } => {
...
): Promise<{ success: boolean; authResult?: AuthResult; error?: { message: string; code: number; authMethod: string } }> => {
  try {
...
export const honoWebSocketHandler = <TEnv extends Env = Env>(
  ...
  return (c: Context<TEnv>): WSEvents => {
    ...
    let request = requestOrError as WebSocketConnectionRequest;
    ...
    return {
      async onOpen(_event, ws) {
        ...
        const openEffect = handleWebSocketOpen(request, uploadServer, flowServer);
      ...
      async onMessage(event, ws) {
        ...
        const messageEffect = handleWebSocketMessage(event.data as string, connection);
      ...
      async onClose() {
        ...
        const closeEffect = handleWebSocketClose(request, uploadServer, flowServer);
