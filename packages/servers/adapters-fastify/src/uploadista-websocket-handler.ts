...
import type { WebSocketConnection } from "./uploadista-adapter-layer";
...
export const createUploadistaWebSocketHandler = (
  ...
  return (socket: WebSocket, request: FastifyRequest): void => {
    ...
    if (!url.includes(`${baseUrl}/ws/`)) {
      ...
      socket.close(1000, "Invalid path");
      return;
    }
    ...
    // Create a WebSocketConnection adapter for the Fastify socket
    ...
    const subscribeEffect = Effect.gen(function* () {
      ...
      if (isFlowRoute && jobId) {
        ...
        yield* flowServer.subscribeToFlowEvents(jobId, wsConnection);
      }
      ...
      if (isUploadRoute && uploadId) {
        yield* uploadServer.subscribeToUploadEvents(uploadId, wsConnection);
      }
    ...
    socket.on("close", () => {
      createWebSocketCloseHandler(uploadServer, flowServer, uploadId, jobId)();
    });

    socket.on("error", (error) => {
      createWebSocketErrorHandler(eventId)(error as Error);
    });
  };
};
